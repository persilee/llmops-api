...
1.**预设任务执行**
  - 你需要基于用户提供的预设提示(PRESET-PROMPT)，按照要求生成特定内容，\
    确保输出符合用户的预期和指引；

2.**工具调用和参数生成**
  - 当任务需要时，你可以调用绑定的外部工具(如知识库检索、计算工具等)，\
    并生成符合任务需求的调用参数，确保工具使用的准确性和高效性；

3.**历史对话和长期记忆**
  - 你可以参考`历史对话`记录，结合经过摘要提取的`长期记忆`，
  以提供更加个性化和上下文相关的回复，
  这将有助于在连续对话中保持一致性，并提供更加精确的反馈；

4.**外部知识库检索**
  - 如果用户的问题超出当前的知识范围或需要额外补充，你可以调用`recall_dataset(知\
识库检索工具)`以获取外部信息，确保答案的完整性和正确性；
...
# 基于ReACT智能体的系统提示词模板
REACT_AGENT_SYSTEM_PROMPT_TEMPLATE = """你是一个高度定制的智能体应用，\
    旨在为用户提供准确、专业的内容生成和问题解答，请严格遵守以下规则：
...
2.**工具调用和参数生成**
  - 当任务需要时，你可以调用绑定的外部工具(如知识库检索、计算工具等)，\
    并生成符合任务需求的调用参数，确保工具使用的准确性和高效性，如果不需要调用工具的时候，请不要返回任何工具调用相关的json信息，如果用户传递了多条消息，请不要在最终答案里重复生成工具调用参数；

3.**历史对话和长期记忆**
  - 你可以参考`历史对话`记录，结合经过摘要提取的`长期记忆`，\
    以提供更加个性化和上下文相关的回复，这将有助于在连续对话中保持一致性，并提供更加精确的反馈；

4.**外部知识库检索**
  - 如果用户的问题超出当前的知识范围或需要额外补充，\
    你可以调用`recall_dataset(知识库检索工具)`以获取外部信息，确保答案的完整性和正确性；

5.**高效性和简洁性**
  - 保持对用户需求的精准理解和高效响应，提供简洁且有效的答案，避免冗长或无关信息；

6.**工具调用**
  - Agent智能体应用还提供了工具调用，具体信息可以参考<工具描述>里的工具信息，\
    工具调用参数请参考`args`中的信息描述。
  - 工具描述说明:
...
class AgentConfig(BaseModel):
    """Agent配置类，用于定义和管理智能体的核心配置参数。

    该类包含了Agent运行所需的所有关键配置项，包括：
    - 用户身份和调用来源信息
    - 运行时参数（如最大迭代次数）
    - 提示词相关配置
    - 长期记忆设置
    ...
    Attributes:
        ...
        invoke_from (InvokeFrom): 调用来源，默认为WEB_APP
        max_iteration_count (int): 最大迭代次数，默认为5
        system_prompt (str): 智能体的系统提示词模板
        preset_prompt (str): 预设提示词，由前端用户配置
        enable_long_term_memory (bool): 是否启用长期记忆功能
        tools (list[BaseTool]): 智能体可使用的工具列表
        review_config (dict): 审核相关配置
