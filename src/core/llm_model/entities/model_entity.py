...
class BaseLanguageModel(LCBaseLanguageModel, ABC):
    ...
    def convert_to_human_message(
        ...
        """将查询文本和图片URL转换为HumanMessage对象。

        该方法根据模型是否支持图片输入，将查询内容转换为适当格式的HumanMessage。
        如果模型不支持图片输入或未提供图片URL，则返回纯文本消息；
        否则返回包含文本和图片的多模态消息。

        Args:
            query: 查询文本内容
            image_urls: 图片URL列表，可选参数，默认为None

        Returns:
            HumanMessage: 转换后的人类消息对象，可能包含纯文本或文本+图片内容
        ...
        # 2.存在图片输入并且支持多模态输入，则按照OpenAI规则转换成人类消息，
        # 如果模型有差异则直接继承重写
