...
class Account(UserMixin, db.Model):
    ...
    @property
    def wechat(self) -> Optional["AccountOAuth"]:
        """获取微信OAuth绑定信息"""
        account_oauth = (
            db.session.query(AccountOAuth)
            .filter(
                AccountOAuth.account_id == self.id,
                AccountOAuth.provider == "wxmp",
            )
            .first()
        )
        if account_oauth:
            return account_oauth.provider

        return ""

    @property
    def github(self) -> Optional["AccountOAuth"]:
        """获取GitHub OAuth绑定信息"""
        account_oauth = (
            db.session.query(AccountOAuth)
            .filter(
                AccountOAuth.account_id == self.id,
                AccountOAuth.provider == "github",
            )
            .first()
        )
        if account_oauth:
